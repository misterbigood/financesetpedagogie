...
/*---------------- Fonction de liste des utilisateurs -------------*/
function liste_users_log( $data_select, $data_where, $data_order, $data_limit )
{
	// Déclaration des variables
	
	
	//Requête
	$data_select<>"" ? $request_string = "SELECT ".$data_select." FROM connexionlog" : $request_string = "SELECT * FROM connexionlog";
	$data_where <>"" ? $request_string .= " WHERE ".$data_where :  FALSE;
	$data_order <>"" ? $request_string .= " ORDER BY ".$data_order :  FALSE;
	$data_limit <>"" ? $request_string .= " LIMIT ".$data_limit : FALSE;
	...
	$request_string_user = "SELECT uniq_id, nom FROM utilisateurs";
	$request_user 	= mysql_query(	$request_string_user );
	
	while($resultat_user=mysql_fetch_array($request_user))
	{
	 $user[utf8_encode($resultat_user['uniq_id'])]=utf8_encode($resultat_user['nom']);
  }
		...
		while($resultat=mysql_fetch_array($request))
		{
			$uniq_id_user			  =	utf8_encode($resultat["uniq_id_user"]);
			$dateconnexion			=	utf8_encode($resultat["dateconnexion"]);
			$ipconnexion			  =	utf8_encode($resultat["ipconnexion"]);
			$statutconnexion		=	utf8_encode($resultat["statutconnexion"]);
			$nom                            =       isset($user[$uniq_id_user]) ? $user[$uniq_id_user]:"Utilisateur suppr.";
						
			$tableau_classe[] 	=                                   array("uniq_id_user" 			=> $uniq_id_user, 
...
function liste_abonnes( $data_select, $data_where, $data_order, $data_limit, $outfile )
{
	// Déclaration des variables
	
	
	//Requête
	$data_select<>"" ? $request_string = "SELECT ".$data_select : $request_string = "SELECT * ";
	$outfile<>"" 	 ? $request_string .= " INTO OUTFILE '".$outfile."' FIELDS TERMINATED BY ';' OPTIONALLY ENCLOSED BY ''" : FALSE;
					   $request_string .= " FROM abonnes";
	$data_where <>"" ? $request_string .= " WHERE ".$data_where :  FALSE;
	$data_order <>"" ? $request_string .= " ORDER BY ".$data_order :  FALSE;
	$data_limit <>"" ? $request_string .= " LIMIT ".$data_limit : FALSE;
	
	$request 	= mysql_query(	$request_string ) or die("Erreur lors du query:".mysql_error());;
	
	if($outfile=="") { // Si le résultat n'est pas envoyé dans un fichier extérieur
		// Traitement de la requête
		while($resultat=mysql_fetch_array($request))
		{
			$id_user			=	utf8_encode($resultat["id_user"]);
...
/* ------------------------Conversion date time vers JJ/MM/AAAA HH:mm-----------------------*/
function datetime_to_fr_with_time($datetime) {
	$tabD = explode("-", substr($datetime, 0, 10) );
	$H = substr($datetime, 11, 5);
	
	
	return ( trim($tabD[2])."/".trim($tabD[1])."/".trim($tabD[0])." ".$H );
}
...
function calcul_date_fin($uniq_id, $date, $max_mail, $delai)
{
	$resultat = liste_abonnes("","actif_user = '1'","dateabonnement DESC","","");
	$resultat2 = mysql_query("SELECT id_envoi FROM envois WHERE uniq_id_li='".$uniq_id."'");
	$nbre_abonnes = count($resultat);
	$nbre_abonnes2= count($resultat2);
	$nbre_envois_restant = floor( ($nbre_abonnes - $nbre_abonnes2) / $max_mail);
	return( date("Y-m-d H:i", strtotime($date) + (60 * $nbre_envois_restant * $delai) ) );
}
