...
{
$data[0] = array ( "numero" =>  bdd_prepare($numero), "li_titre" => bdd_prepare($li_titre), "actif_li" => $actif_li, "dateactivationenvoi" => bdd_prepare($dateactivationenvoi), "lienpdffr" => bdd_prepare($lienfrancais), "lienpdfen" => bdd_prepare($lienanglais), "dataseries" => mysql_real_escape_string(serialize($datatoserialize)) );

// Si aucune erreur sur les variables
$bdderror["bdd"] = ajout_li( $data );
}
else
{
$data[0] = array ( "numero" =>  bdd_prepare($numero), "li_titre" => bdd_prepare($li_titre), "actif_li" => $actif_li, "dateactivationenvoi" => "0000-00-00 00:00:00", "lienpdffr" => bdd_prepare($lienfrancais), "lienpdfen" => bdd_prepare($lienanglais), "dataseries" => mysql_real_escape_string(serialize($datatoserialize)) );
